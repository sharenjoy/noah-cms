...
namespace Sharenjoy\NoahCms\Models\Survey;
        ...
        return [
            'left' => [
                ...
                'type' => FormSection::make()->schema([
                    Select::make('type')
                        ->label(__('noah-cms::noah-cms.survey.title.type'))
                        ->required()
                        ->reactive()
                        ->options(
                            collect(config('noah-cms.survey.question.types'))
                                ->mapWithKeys(fn($label, $key) => [
                                    $key => __('noah-cms::noah-cms.survey.title.' . strtolower($label))
                                ])
                                ->toArray()
                        ),
                ]),
                'options' => FormSection::make()->schema([
                    TagsInput::make('options')
                        ->label(__('noah-cms::noah-cms.survey.title.options'))
                        ->placeholder('New option')
                        ->helperText(__('noah-cms::noah-cms.survey.help.options'))
                        ->required()
                        ->visible(fn(Get $get) => $get('type') == 'radio' || $get('type') == 'multiselect'),
                ])->visible(fn(Get $get) => $get('type') == 'radio' || $get('type') == 'multiselect'),
                'price' => FormSection::make()->schema([
                    TextInput::make('price')
                        ->label(__('noah-cms::noah-cms.survey.title.price'))
                        ->placeholder('300')
                        ->helperText(__('noah-cms::noah-cms.survey.help.price'))
                        ->required()
                        ->visible(fn(Get $get) => $get('type') == 'price'),
                ])->visible(fn(Get $get) => $get('type') == 'price'),
                'rules' => FormSection::make()->schema([
                    TagsInput::make('rules')
                        ->label(__('noah-cms::noah-cms.survey.title.rules'))
                        ->placeholder('New rule')
                        ->helperText(__('noah-cms::noah-cms.survey.help.rules')),
                ]),
            ],
            'right' => [
                // 'album' => [],
        ...
        return [
            ...
            'type' => TextColumn::make('type')
                ...
                ->state(function ($record) {
                    return __('noah-cms::noah-cms.survey.title.' . $record->type);
                })
